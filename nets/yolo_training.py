...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        elif self.loss_type == "viou":
            scale_diff = torch.abs(torch.log((pred[:, 2:] + 1e-6) / (target[:, 2:] + 1e-6)))
            scale_penalty = scale_diff.mean(dim=1)
            loss = 1 - iou + scale_penalty
...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, fp16, strides=[8, 16, 32]):
        ...
        self.iou_loss           = IOUloss(reduction="none", loss_type="viou")
